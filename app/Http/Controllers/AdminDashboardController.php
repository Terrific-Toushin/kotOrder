...
class AdminDashboardController extends Controller
{
    public function index(){
        ...
        $users = UserPrivileges::selectRaw('count(user_id) as total, SUM(CASE WHEN user_type = "operator" THEN 1 ELSE 0 END) as operator_count,
                            ...
                            SUM(CASE WHEN user_type = "kitchen" THEN 1 ELSE 0 END) as kitchen_count')->leftJoin('users', 'user_privileges.user_id', '=', 'users.id')->where('users.email','!=','toushin.java@gmail.com')->where('users.PropertyID','!=', Auth::user()->PropertyID)
            ->first();
        $panding_kots_count = DB::table('order_kot')->where('cancel', '=', 'N')->where('PropertyID','=',Auth::user()->PropertyID)->where(function ($q) {$q->where('status', '=', '1')->orWhere('status', '=', '4');})->count();
        $kitchen_complete_kot_count = DB::table('order_kot')->where('status', '=', '3')->where('PropertyID','=',Auth::user()->PropertyID)->where('cancel', '=', 'N')->count();
        $total_kots_count = DB::table('order_kot')->where('cancel', '=', 'N')->where('PropertyID','=',Auth::user()->PropertyID)->count();
        $cash_print_count = DB::table('order_kot')->where('status', '=', '2')->where('PropertyID','=',Auth::user()->PropertyID)->where('cancel', '=', 'N')->count();
        ...
        $outlets_count=DB::table('order_kot')->selectRaw('outlet, SUM(CASE WHEN cancel = "N" AND (status = "1" OR status = "4") THEN 1 ELSE 0 END) as panding_kots_count,
                                     ...
                                     SUM(CASE WHEN cancel = "Y" THEN 1 ELSE 0 END) as kot_cancel_count')->where('PropertyID','=',Auth::user()->PropertyID)->groupBy('outlet')->get();
    ...
    public function redirects(){
        ...
        if ($role == 'admin'){
            ...
            return redirect()->route('addUser','new');
        }
    }
